...
fr=imread(strcat(path,source(1).name));
...
for n = 8:(length(source)-2)              %there will be false route line,so it only include the pictures which has the car
    ...
    n
    source(n).name
    fr = imread(strcat(path,source(n).name));       % read in frame
    ...
    for i=1:height                        %%%%%%%%%%%%%search each pixal of one image, if it is in the C ge gaosimoxing, it is belong to the background,and undate the background.
        for j=1:width                     %%%%%%%%%% If it is not in the C ge gaosimoxing, create a new gaosi and replace the least possible gaosi. Finally the first several gaosi is background, and the last several is foreground 
